<style>
    ...
    .product-section {
        margin-top: 3rem; /* Pushes products slightly below the carousel */
        width: 110%; /* Increased width */
        max-width: 100vw; /* Ensures it doesn't overflow screen */
        margin-left: -5%; /* Adjusts for the increased width */
        margin-right: -5%;
    }
</style>
...
<section class="py-3">
    <div class="container">
        ...
        <!-- Product Section with More Width -->
        <div class="row justify-content-center mt-5 product-section">
            <div class="col-lg-12"> <!-- Full width for 4 products -->
                <div class="card card-outline rounded-0">
                    <div class="card-body">
                        <div class="row d-flex justify-content-center">
                            <?php 
                                $qry = $conn->query("SELECT *, 
                                    (COALESCE((SELECT SUM(quantity) FROM `stock_list` WHERE product_id = product_list.id AND (expiration IS NULL OR DATE(expiration) > '".date("Y-m-d")."')), 0) 
                                    - COALESCE((SELECT SUM(quantity) FROM `order_items` WHERE product_id = product_list.id), 0)) AS `available` 
                                    FROM `product_list` 
                                    WHERE (COALESCE((SELECT SUM(quantity) FROM `stock_list` WHERE product_id = product_list.id AND (expiration IS NULL OR DATE(expiration) > '".date("Y-m-d")."')), 0) 
                            ...
                            ?>
                            <div class="col-lg-3 col-md-6 col-sm-12 mb-3"> <!-- Ensures 4 products show up in one row -->
                        ...
                        <div class="text-center py-1">
                            <a href="./?p=products" class="btn btn-lg btn-default text-light bg-gradient-maroon col-lg-4 col-md-6 col-sm-12 col-xs-12 rounded-pill">View More Items</a>
                        </div>
                    </div>
                </div>
            </div>
